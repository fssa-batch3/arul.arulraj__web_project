...
const signIn = (e) => {
  ...
  const exist = JSON.parse(localStorage.getItem('user_data')).some((data) => data.user_number.toLowerCase() == user_number && data.user_password == user_password);
  if (!exist) {
...
const trainerregistration = (e) => {
  ...
  const exist = JSON.parse(localStorage.getItem('trainer_data')).some((data) => data.trainer_number.toLowerCase() == trainer_number && data.trainer_password == trainer_password);
  if (!exist) {
    alert('Incorrect login credentials');
  } else {
